package com.beaunolten.rise.ui.plant
...
import com.beaunolten.rise.ui.theme.BerryA
import com.beaunolten.rise.ui.theme.BerryB
import com.beaunolten.rise.ui.theme.PotA
import com.beaunolten.rise.ui.theme.PotB
import com.beaunolten.rise.ui.theme.PotRim
import com.beaunolten.rise.ui.theme.Stem
import com.beaunolten.rise.ui.theme.StemTip
...
private class PlantState(seed: Int, val maxGeneration: Int) {
    ...
    fun draw(ds: DrawScope, sway: Float) {
        val scale = ds.size.height / 196f
        val seg = 5f * scale
        val baseX = ds.size.width / 2f
        val baseY = ds.size.height - 6f * scale
        ...
        for (i in word.indices) {
            when (word[i]) {
                ...
                'A' -> { val lt = if (lerpOn > 0) t else 1f; ds.drawCircle(BerryA, seg * lt, Offset(x, y)) }
                'B' -> { val lt = if (lerpOn > 0) t else 1f; ds.drawCircle(BerryB, seg * lt, Offset(x, y)) }
            }
        }
    }
}
...
fun PlantCanvas(generations: Int, modifier: Modifier = Modifier, seed: Int = remember { Random.nextInt() }) {
    ...
    var nowMs by remember(plant) { mutableStateOf(0L) }
    ...
    Canvas(modifier) {
        val elapsed = nowMs / 1000f
        val swayAmp = if (plant.finished) 0.025f else 0.012f
        val sway = (sin((elapsed * 0.9f).toDouble()) * swayAmp).toFloat()
        plant.draw(this, sway)
    }
}
...
fun PlantWithPot(generations: Int, modifier: Modifier = Modifier) {
    Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = modifier) {
        PlantCanvas(generations, Modifier.width(210.dp).height(196.dp))
        Spacer(
            Modifier
                .offset(y = (-6).dp)
                .width(96.dp)
